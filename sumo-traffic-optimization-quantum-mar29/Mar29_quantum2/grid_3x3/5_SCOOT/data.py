"""
Canonical 3x3 Seattle SCOOT evaluation.
...
The working 3x3 SCOOT detector, split, offset, regional-cycle, queue-prediction,
and signal-application logic is preserved.
"""
...
def apply_target_offsets():
    ...
    scoot_node_state[REFERENCE_TLS]["offset"] = 0

    for tls in TLS_ORDER:

        if tls == REFERENCE_TLS:
            continue
...
# -----------------------
# CANONICAL SEATTLE METRICS + SCOOT DATA STRUCTURES
# -----------------------
depart_time = {}
depart_hour = {}
last_waiting_time = {}
...
def simStep(num_times=1):
    """
    Advance SUMO and collect the canonical Seattle metrics while preserving
    SCOOT's queue and virtual-detector updates below.
    """
    for _ in range(num_times):
        ...
        # Track only vehicles departing after the hourly warm-up.
        ...
        # Attribute completions to their departure hour.
...
print("\n===== 3x3 SCOOT SEATTLE PERFORMANCE METRICS =====")
