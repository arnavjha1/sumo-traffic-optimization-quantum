...
from annealer_hyper import quantum_decision, set_run_seed
...
HYPER_TYPE = sys.argv[2].lower()
HYPER_VALUE = float(sys.argv[3])
...
TEST_GAMMA = BASE_GAMMA
TEST_BETA = BASE_BETA
TEST_P = BASE_P
TEST_SHOTS = BASE_SHOTS

if HYPER_TYPE == "gamma":
    TEST_GAMMA = HYPER_VALUE

elif HYPER_TYPE == "beta":
    TEST_BETA = HYPER_VALUE

elif HYPER_TYPE == "p":
    TEST_P = int(HYPER_VALUE)

elif HYPER_TYPE == "shots":
    TEST_SHOTS = int(HYPER_VALUE)

else:
    raise ValueError(
        "HYPER_TYPE must be gamma, beta, p, or shots"
    )
...
print(f"Hyperparameter: {HYPER_TYPE}")
print(f"Test value:     {HYPER_VALUE}")
print(f"Gamma:          {TEST_GAMMA:.6f}")
print(f"Beta:           {TEST_BETA:.6f}")
print(f"p:              {TEST_P}")
...
tIndex = TLS_ORDER.copy()  # Maintain a consistent index mapping for TLS
...
while traci.simulation.getTime() < END_TIME:
    ...
    # Convert downstream-aware biases to a flat list (length 9)
    ...
    assert tIndex == TLS_ORDER
    assert len(bias_list) == 9
    assert len(prev_state) == 9
    ...
    if current_time == 1:
        ...
        print("Bias count:", len(bias_list))
