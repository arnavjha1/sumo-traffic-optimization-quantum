...
SIMULATION_FILE = SIMULATION_DIR / "3_quantum.py"
ANNEALER_DIR = SIMULATION_DIR / "3_quantum"
SUMO_CONFIG_FILE = BASE_DIR / "sim2x2_data.sumocfg"
OUTPUT_DIR = BASE_DIR / "seattle_quantum_data"

JSON_OUTPUT_FILE = OUTPUT_DIR / "quantum_simulation_results.json"
TXT_OUTPUT_FILE = OUTPUT_DIR / "quantum_simulation_summary.txt"
CSV_OUTPUT_FILE = OUTPUT_DIR / "quantum_hourly_summary.csv"
...
# This matches the output emitted by PerformanceTracker.print_results() in
...
def build_subprocess_environment():
    """Make both simulation_metrics.py and annealer_quantum.py importable."""
    environment = os.environ.copy()
    python_paths = [str(SIMULATION_DIR), str(ANNEALER_DIR)]
...
def extract_hourly_metrics(output):
    ...
    if len(hourly_metrics) != 24:
        raise ValueError(
            f"Expected 24 hourly result blocks in the console output, "
        ...
        for hour in hourly_summary:
            ...
            for metric_name in METRIC_NAMES:
                ...
                row.update({
                    ...
                    f"{metric_name}_standard_deviation": stats["standard_deviation"],
...
def write_text_summary(results):
    with TXT_OUTPUT_FILE.open("w", encoding="utf-8") as output_file:
        output_file.write("===== SEATTLE QUANTUM SIMULATION SUMMARY =====\n\n")
        ...
        output_file.write("===== HOURLY MEAN +/- SAMPLE STANDARD DEVIATION =====\n\n")
...
def save_results(runs):
    ...
    # Checkpoint after every run so a long batch still leaves useful results.
    write_json(results)
...
def run_simulations():
    ...
    if not (ANNEALER_DIR / "annealer_quantum.py").is_file():
        raise FileNotFoundError(f"Quantum annealer module not found: {ANNEALER_DIR}")
    ...
    for run_number in range(1, NUM_RUNS + 1):
        print(f"===== Quantum run {run_number}/{NUM_RUNS} =====", flush=True)
        output = ""
        ...
        try:
            completed = subprocess.run(
                ...
                encoding="utf-8",
                errors="replace",
                # 3_quantum.py refers to sim2x2_data.sumocfg relative to the repo root.
                cwd=BASE_DIR,
...
def print_final_summary(results):
    ...
    print(
        f"\nCompleted {results['num_runs_successful']}/{results['num_runs_attempted']} "
        "runs successfully."
    )
    print("\n===== FILES SAVED =====")
