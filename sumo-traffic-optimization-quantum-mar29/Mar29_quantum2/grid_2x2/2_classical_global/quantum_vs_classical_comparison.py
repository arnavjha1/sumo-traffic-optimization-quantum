...
from annealer_quantum import quantum_decision as quantum_decision
from annealer_classical import quantum_decision as classical_decision
...
x_i = [[] for _ in range(NUM_TLS)]  # Quantum controller state (active controller)
classical_x_i = [[] for _ in range(NUM_TLS)]  # Classical annealer state for comparison
...
while traci.simulation.getTime() < END_TIME:
    ...
    # Get both optimizer decisions from the exact same traffic state
    quantum_bitstring = quantum_decision(
        bias_list,
        prev_state,
        neighbor_indices,
        coupling_strength=2
    ...
    classical_bitstring = classical_decision(
        bias_list,
        prev_state,
        neighbor_indices,
        coupling_strength=2
    ...
    print(
        f"[COMPARE] t={current_time} "
        f"bias={ [round(x,2) for x in bias_list] } "
        f"prev={prev_state} "
        f"QAOA={quantum_bitstring} "
        f"CLASSICAL={classical_bitstring}"
    )

    # Update quantum state (active controller)
    for idx, tls in enumerate(TLS_ORDER):
        x_i[idx].append(
            1 if quantum_bitstring[idx] == '1' else -1
        )

        classical_x_i[idx].append(
            1 if classical_bitstring[idx] == '1' else -1
        )

    # Compare how often decisions disagree
    if quantum_bitstring != classical_bitstring:
        print(
            f"[DIFFERENCE] t={current_time} "
            f"QAOA={quantum_bitstring} "
            f"CLASSICAL={classical_bitstring}"
        )
...
for idx, tls in enumerate(TLS_ORDER):
    ...
    print(
        f"{tls}: "
        f"different decisions = {differences} / {len(x_i[idx])}"
    )
