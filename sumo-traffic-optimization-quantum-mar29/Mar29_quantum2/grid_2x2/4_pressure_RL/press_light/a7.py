...
def get_presslight_state(tls):
    ...
    for signal_index, link_group in enumerate(controlled_links):
        ...
        for link in link_group:
            ...
            if signal_index < len(PHASE_NS) and PHASE_NS[signal_index] in ("G", "g"):

                ns_upstream += upstream_queue
                ns_downstream += downstream_queue

            if signal_index < len(PHASE_EW) and PHASE_EW[signal_index] in ("G", "g"):
...
def get_presslight_reward(tls):

    state = get_presslight_state(tls)

    ns_upstream = state[0]
    ns_downstream = state[1]

    ew_upstream = state[2]
    ew_downstream = state[3]
