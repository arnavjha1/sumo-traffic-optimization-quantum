...
LAMBDA_SWITCHING_PENALTY = 20
coupling_bias = 0

x_i = [[] for _ in range(NUM_TLS)]
sampler = LeapHybridSampler()   # Hybrid solver for BQM

def optimize_x_i(tls_index, bias_i):
    """Use D-Wave BQM to choose +1/-1 phase."""
    # Convert previous state to spin variable
    if len(x_i[tls_index]) == 0:
        x_i[tls_index].append(1 if bias_i >= 0 else -1)
        return

    prev_x = x_i[tls_index][-1]

    # Build spin BQM: x = +1/-1
    bqm = dimod.BinaryQuadraticModel('SPIN')

    # Linear bias: preference for switching or staying
    bqm.add_linear('x', -bias_i * prev_x)

    # Switching penalty
    bqm.add_linear('s', LAMBDA_SWITCHING_PENALTY)
    ...
    for neighbor_tls in TLS_NEIGHBORS[tls_index][1:]:
        ...
        if len(x_i[neighbor_index]) > 0:
            neighbor_x = x_i[neighbor_index][-1]
            bqm.add_interaction('x', f'n{neighbor_index}', -coupling_bias * neighbor_x * prev_x)

    # Solve BQM
    sampleset = sampler.sample(bqm)
    sample = sampleset.first.sample
    # Decide new phase: if 'x' in sample, convert to ±1 spin
    new_phase = sample.get('x', 1)
    x_i[tls_index].append(new_phase)
